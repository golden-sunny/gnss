...
DATA_DIR = ROOT / "data"
SUMMARY_CSV = DATA_DIR / "exam-5.7-trop_summary.csv"
DETAIL_CSV = DATA_DIR / "exam-5.7-trop.csv"
OUTPUT_BASE = DATA_DIR / "exam-5.7-trop_science"
G10_OUTPUT_BASE = DATA_DIR / "exam-5.7-trop_g10_science"
...
def plot_g10(detail: pd.DataFrame) -> tuple[Path, Path]:
    ...
    if g10.empty:
        raise ValueError("未在 exam-5.7-trop.csv 中找到 G10 卫星记录。")

    g10_plot = insert_gap_rows(g10, "epoch", ["slant_delay_m"])
    ...
    ax.plot(
        g10_plot["epoch"],
        g10_plot["slant_delay_m"],
        color="black",
        linestyle="-",
        label="G10卫星斜路径对流层延迟",
    )
    ax.set_title("G10卫星 Saastamoinen 对流层延迟", fontproperties=CN_FONT)
    ax.set_xlabel("GPST时间", fontproperties=CN_FONT)
    ax.set_ylabel("斜路径延迟 / m", fontproperties=CN_FONT)
    ax.legend(loc="upper right", prop=CN_FONT)
    apply_time_axis(ax, start, end)

    fig.suptitle("练习5.7 G10卫星对流层延迟", x=0.08, y=1.03, ha="left", fontsize=10, fontweight="bold", fontproperties=CN_FONT)
    fig.text(
        0.08,
        0.985,
        (
...
def main() -> None:
    ...
    summary_plot = insert_gap_rows(summary, "epoch", ["mean_trop_m", "min_trop_m", "max_trop_m"])
    ...
    ax.fill_between(
        summary_plot["epoch"],
        summary_plot["min_trop_m"],
        summary_plot["max_trop_m"],
        color="#d9d9d9",
        alpha=0.8,
        label="可见GPS卫星延迟范围",
    )
    ...
    ax.set_title("全天 Saastamoinen 对流层延迟", fontproperties=CN_FONT)
    ax.set_xlabel("GPST时间", fontproperties=CN_FONT)
    ax.set_ylabel("斜路径延迟 / m", fontproperties=CN_FONT)
    ax.legend(loc="upper right", prop=CN_FONT)
    apply_time_axis(ax, start, end)

    fig.suptitle("练习5.7 对流层延迟全天变化", x=0.08, y=1.03, ha="left", fontsize=10, fontweight="bold", fontproperties=CN_FONT)
    fig.text(
        0.08,
        0.985,
        (
            f"历元数 = {len(summary)}，平均可见GPS卫星数 = {summary['sat_count'].mean():.1f}，"
            f"平均高度角 = {summary['mean_elevation_deg'].mean():.1f}°"
        ),
        ha="left",
        va="top",
        fontsize=8,
